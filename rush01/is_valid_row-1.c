...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/14 14:30:27 by rynitta           #+#    #+#             */
/*   Updated: 2025/09/14 14:30:30 by rynitta          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	_check_row_direction_left(int map[][9], int len_line, \
...
static int	_check_row_direction_right(int map[][9], int len_line, \
	int num_viewable_box[][9], int i_row)
{
	int	count_viewable_box_;
	int	current_max_height_;
...
static int	_is_unique_height(int map[][9], int len_line, int i_row)
{
	bool	is_used_[10];
	int		i;
	...
	int num_viewable_box[][9], int i_row)
{
	if (_is_unique_height(map, len_line, i_row) == false
		|| _check_row_direction_left(map, len_line, \
			num_viewable_box, i_row) == NG \
		|| _check_row_direction_right(map, len_line, \
			num_viewable_box, i_row) == NG)
		return (false);
	return (true);
}
